...
export default function Navbar() { const [mode, setMode] = useThemeSwitcher() 
...
const CustomeLink = ({href, title, className=""}) =>{ 
    ...
    return( 
        <Link href={href} className={`${className} relative group`}>
                {title}
                <span 
                className={`h-[1px] inline-block bg-dark absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 dark:bg-light ${router.asPath === href ? 'w-full' : 'w-0'}`}>
                    &nbsp;</span>
            </Link>
        )
    }
    const CustomeMobileLink = ({href, title, className="", toggle }) =>{
        ...
        return(
            ...
            >{title} <span className={`h-[1px] inline-block bg-light absolute
              left-0 -bottom-0.5 group-hover:w-full transition-[width] ease
              duration-300 dark:bg-dark ${router.asPath ===
              href ? 'w-full' : 'w-0'}`}>
                    &nbsp;</span>
            </button>
        )
  ...
  return (
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8">
        ...
        
        {
            ...
            className="min-w-[70vw] z-30 flex flex-col justify-between items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-sm py-32">
                <nav className='flex items-center flex-col justify-center'>
                    <CustomeMobileLink href="/" title={'Home'} className="" toggle={handelClick}/>
                    <CustomeMobileLink href="/about" title={'About'} className="" toggle={handelClick} />
                    <CustomeMobileLink href="/projects" title={'Projects'} className="" toggle={handelClick} />
                    <CustomeMobileLink href="/articles" title={'Articles'} className="" toggle={handelClick} />
                </nav>
                <nav className="flex items-center justify-center flex-wrap mt-4">
                    ...
                    <motion.a href="https://twitter.com/" target={"_blank"}
                        className="w-6 mx-3 sm:mx-1 bg-light rounded-full dark:bg-black"
                        whileHover={{y:-2}}
                        whileTap={{scale:0.9}}
                    >
                        <GithubIcon />
